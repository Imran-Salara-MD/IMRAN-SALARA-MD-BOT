async function antistatusCommand(sock, from, msg, isAdmin, botData, saveBotData, args, userId) {
    ...
    if (!botData.antiStatusGroups[userId]) botData.antiStatusGroups[userId] = {};

    if (action === 'on') {
        botData.antiStatusGroups[userId][from] = true;
        saveBotData();
        ...
        delete botData.antiStatusGroups[userId][from];
        saveBotData();
